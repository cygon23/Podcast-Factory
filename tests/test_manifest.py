...
def test_lesson_with_matching_hash_and_success_does_not_need_processing(manifest):
    ...
    assert manifest.needs_processing(category_number=30, lesson=lesson, engine="null") is False
...
def test_manifest_persists_across_instances(tmp_path):
    ...
    assert m2.needs_processing(category_number=30, lesson=lesson, engine="null") is False
...
def test_plan_run_reports_new_changed_and_skipped(manifest):
    ...
    plan = manifest.plan_run([category], engine="null")
